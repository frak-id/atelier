...
/**
 * The standalone-toolset surface: toolsets not produced by a toolbox (manual
 * builds + live-sandbox captures). `filter` narrows the global toolset list;
 * toolbox artifacts (`tb/…`) are rendered inline with their toolbox instead.
 */
export function ToolsetsSection({
  filter,
}: {
  filter?: (toolset: ToolsetEntry) => boolean;
}) {
  const { data, isPending, isError, error } = useQuery(toolsetsListQuery());
  const [buildOpen, setBuildOpen] = useState(false);
  const toolsets = filter ? data?.filter(filter) : data;

  return (
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          <h2 className="text-sm font-semibold">Standalone toolsets</h2>
          <p className="text-sm text-muted-foreground">
            Artifacts not tied to a toolbox — one-off manual builds and private
            captures from a live sandbox. Reference them directly in a spec.
          </p>
        </div>
      ...
      {isPending ? (
        ...
        <p className="text-sm text-muted-foreground">No standalone toolsets.</p>
      ) : (
