...
function extractSandboxId(host: string): { type: "vscode" | "opencode"; id: string } | null {
  const patterns = [
    { regex: new RegExp(`^sandbox-([^.]+)\\.${BASE_DOMAIN.replace(/\./g, "\\.")}$`), type: "vscode" as const },
    { regex: new RegExp(`^opencode-([^.]+)\\.${BASE_DOMAIN.replace(/\./g, "\\.")}$`), type: "opencode" as const },
  ];
...
export const proxyRoutes = new Elysia({ prefix: "/internal" })
  .get(
    "/verify-domain",
    ({ query }) => {
      ...
      const subdomainMatch = domain.match(new RegExp(`^([^.]+)\\.${BASE_DOMAIN.replace(/\./g, "\\.")}$`));
