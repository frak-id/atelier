...
export class SystemAiService {
  ...
  private buildDescriptionPrompt(
    ...
    return [
      ...
      `workspace.`,
      ``,
      `IMPORTANT CONTEXT: This description is used by an AI`,
      `agent to pick the right workspace for a given task.`,
      `When a user asks "update the landing page for project`,
      `X", the agent reads ALL workspace descriptions and`,
      `selects the best match. Your description must make it`,
      `unambiguous which project/product this workspace`,
      `belongs to and what kind of work happens here.`,
      ``,
      `Focus on: the project/product name, what it does`,
      `(e.g. "marketing site", "REST API", "mobile app"),`,
      `key technologies/frameworks, and the repo structure`,
      `if multiple repos are involved.`,
      ``,
