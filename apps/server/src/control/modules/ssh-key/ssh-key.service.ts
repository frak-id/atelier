...
/**
 * SSH public keys → sandbox SSH access. `getValidPublicKeys()` used to be
 * pulled directly at sandbox boot (v1 leak #2); now it's an explicit input
 * enrichment resolves into `RuntimeCreateOptions.authorizedKeys` at the seam.
 */
export class SshKeyService {
  ...
  create(options: CreateOptions): SshKey {
    ...
    return this.sshKeyRepository.create({
      ...options,
      fingerprint: this.computeFingerprint(options.publicKey),
    });
  }

  delete(id: string, userId: string): void {
    const existing = this.sshKeyRepository.getById(id);
    if (!existing) throw new NotFoundError("SshKey", id);
    if (existing.userId !== userId) {
      throw new ValidationError("Cannot delete another user's SSH key");
    }
  ...
  private computeFingerprint(publicKey: string): string {
    ...
    if (!keyDataBase64) throw new ValidationError("Invalid public key format");
